                ...
                <div class="row">
                    ...
                    if (isset($_GET['search']) && !empty($_GET['search'])) {
                        ...
                        if (isset($_GET['type']) && !empty($_GET['type'])) {
                            ...
                            $selectedTypes = is_array($selectedTypes) ? $selectedTypes : [$selectedTypes];
                            $placeholders = implode(',', array_fill(0, count($selectedTypes), '?'));
                            $sql = "SELECT prd_ID, prd_name, prd_img, prd_price FROM products WHERE cate_ID IN ($placeholders) LIMIT $products_per_page OFFSET $offset";
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param(str_repeat('i', count($selectedTypes)), ...$selectedTypes);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            ...
                            $sql = "SELECT prd_ID, prd_name, prd_img, prd_price FROM products LIMIT $products_per_page OFFSET $offset";
